...
<div x-data="CartSidebar" x-init="$store.refs.cart = $data"
     class="bg-white border-r border-gray-200 shadow-sm z-40 
            w-full h-auto 
            sm:fixed sm:top-0 sm:left-0 sm:h-screen sm:w-80 sm:flex sm:flex-col">
  ...
  <!-- Items -->
  <div class="flex-1 overflow-y-auto p-4 space-y-2 max-h-64 sm:max-h-full">
    ...
    <template x-for="(item, key, index) in cart" :key="key">
      <div class="flex justify-between items-center p-2 rounded-md"
        ...
        <div>
          <p class="font-medium text-sm truncate" x-text="item.name"></p>
          <p class="text-xs text-gray-400" 
             x-text="item.variant ? `${item.variant} x${item.unit_qty}` : `Qty: ${item.unit_qty}`">
          </p>
        </div>
...
<!-- Navbar -->
<nav class="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-30 sm:pl-80">
  <div class="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
    <div class="flex items-center gap-2">
      <i class="fas a-store text-purple-600 fas fa-store"></i>
      <span class="text-base font-semibold text-gray-700">POS</span>
      <span x-text="$store.mode.current.toUpperCase()" 
            class="ml-3 px-2 py-1 text-xs rounded bg-purple-100 text-purple-600 font-semibold">
      </span>
    </div>
...
<!-- Product Grid -->
<div x-data="ProductGrid" x-init="$store.refs.grid = $data" class="p-6 sm:pl-80">
  <!-- Sticky Search Bar -->
...
document.addEventListener('alpine:init', () => {
  // ----------------- Refs Store (safe component handles) -----------------
  ...
  Alpine.store('cart', {
    ...
    add(product, variant = null) {
      ...
      else this.items[key] = { ...v, name: product.name, product_id: product.id, variant: v.unit_type ?? null, unit_qty: v.unit_qty, quantity: 1 };
      this.save();
    },
    ...
    checkout(method) {
      ...
      if (navigator.onLine) {
        ...
        .then(data => {
          ...
          if (data.order_id) window.location.href = `/receipt/${data.order_id}`;
          Alpine.store('cart').clear();
        })
  ...
  document.addEventListener('keydown', (e) => {
    // Let native typing work in inputs/textareas
    ...
    // --- Search as you type (Product Mode) ---
    ...
    // --- Backspace edits search (Product Mode) ---
