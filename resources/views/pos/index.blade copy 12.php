...
main {
    flex: 1;
    overflow: hidden; /* Ensure no overflow within the main container */
}
        * {
            margin: 0;
            padding: 0;
...
<main class="flex flex-col md:flex-row relative h-screen">
    ...
    <div class="cart-fixed bg-white p-4 mb-4 rounded shadow-md relative">
        ...
        <div class="absolute bottom-7 left-0 right-0 ">

        <div class="button-container p-3 bg-gray-100 flex justify-center">
...
<script>
      function closeModal() {
            document.getElementById('variantModal').classList.add('hidden'); // Hide modal
        }
...
function addProductToCart() {
    console.log('Adding product to cart:', selectedProduct, selectedVariant);
}
    ...
    quantityInputs.forEach(input => {
        input.addEventListener('change', function() {
            ...
            updateItemQuantity(this.dataset.itemId, this.value);
        });
    });
});
        ...
        <input type="number" value="${item.quantity}" class="quantity-input" min="1" data-item-id="${item.id}">
    </td>
