    ...
    <div x-data="cartSidebar()" class="bg-white border-r border-gray-200 shadow-xl z-40 w-full md:w-[420px] lg:w-[450px] flex flex-col h-full relative transition-all">
      ...
      <div class="p-4 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-20">
        ...
        <div class="grid grid-cols-3 gap-2">
          ...
          <button @click="pay('bank_transfer')" :disabled="count === 0 || loading" class="relative overflow-hidden flex flex-col items-center justify-center py-3 px-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg shadow-md transition-all active:scale-95 group">
    ...
    function syncManager(){
      return {
        ...
        init(){
          ...
          if(this.online && this.queue.length > 0) setTimeout(()=> this.processQueue(), 2000);
        },
        ...
        async processQueue(){
          ...
          const payload = this.queue[0]; // Peek
          ...
          try {
            ...
            if(res.ok){
              // 1. Success: Remove from queue
              this.queue.shift();
              this.saveQueue();
              window.dispatchEvent(new CustomEvent('notify',{detail:{message:`☁️ Synced: ₦${money(data.total || payload.total)}`, type:'success'}}));
              
              // 2. Loop if more exist
              if(this.queue.length > 0){
                setTimeout(()=> { this.syncing = false; this.processQueue(); }, 500);
              } else {
                this.syncing = false;
              }

            } else if (res.status === 422) {
               // 3. Validation Error (e.g., Stock changed): Do NOT retry forever.
               // Move to failed queue so user can review manually.
               console.error('Validation Error', data);
               this.queue.shift();
               this.failedQueue.push({ ...payload, error: data.message });
               this.saveQueue();
               this.saveFailed();
               window.dispatchEvent(new CustomEvent('notify',{detail:{message:`❌ Order Rejected: ${data.message}`, type:'error'}}));
               this.syncing = false;
               this.processQueue(); // Try next one
            } else {
               // 4. Server Error (500): Keep in queue, retry later
