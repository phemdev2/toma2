...
@php
  $user = auth()->user();
  // Force Title Case for User Name
  $rawName = $user ? $user->name : 'Guest';
  $userName = ucwords(strtolower($rawName));
  ...
  $preparedProducts = $productsRaw->map(function ($product) use ($storeId) {
    // Get stock specific to this store
...
<body class="bg-slate-100 font-sans text-gray-800 h-[100dvh] overflow-hidden flex flex-col no-select" 
  ...
  <nav class="bg-slate-900 text-white h-14 md:h-12 flex-none z-50 shadow-md flex justify-between items-center px-3 md:px-4">
    ...
    <div class="flex items-center gap-2 md:gap-4">
        <button @click="$dispatch('open-mobile-link')" class="hidden md:flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold border border-slate-700 transition-all mr-2 shadow-sm">
        ...
        <div class="flex items-center gap-2 md:gap-4 text-xs" x-data="syncManager()">
            ...
            <div x-show="queueCount > 0" x-cloak 
                 ...
                 @click="process" 
                 @contextmenu.prevent="forceClear"
                 title="Left Click: Sync Now | Right Click: Force Clear Queue">
        ...
        <!-- UPDATES HISTORY LOG -->
        <div x-data="updateLogModal" class="relative mr-2">
            <button @click="openModal" class="relative text-gray-400 hover:text-white transition-colors p-1">
                <i class="fas fa-history text-lg"></i>
                <span x-show="unread > 0" x-transition.scale class="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full shadow-sm" x-text="unread"></span>
            </button>
            <!-- LOG MODAL -->
            <div x-show="show" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" @click.self="close">
                <div class="bg-white rounded-xl shadow-2xl w-full max-w-md h-[80vh] flex flex-col animate-fade-in overflow-hidden">
                    <div class="p-4 border-b bg-gray-50 flex justify-between items-center">
                        <h3 class="font-bold text-gray-800"><i class="fas fa-history text-purple-600 mr-2"></i> Update Log</h3>
                        <button @click="close" class="text-gray-400 hover:text-red-500"><i class="fas fa-times"></i></button>
                    </div>
                    <div class="flex-1 overflow-y-auto p-0 custom-scrollbar bg-slate-50">
                        <template x-if="logs.length === 0">
                            <div class="flex flex-col items-center justify-center h-full text-gray-400 opacity-60">
                                ...
                                <p class="text-xs">No updates received yet today.</p>
                            </div>
                        </template>
                        <template x-for="(log, index) in logs" :key="index">
                            <div class="p-4 border-b border-gray-100 bg-white hover:bg-gray-50 transition-colors">
                                <div class="flex justify-between items-start mb-1">
                                    <span class="font-bold text-sm text-gray-800" x-text="log.name"></span>
                                    <span class="text-[10px] text-gray-400 font-mono" x-text="log.time"></span>
                                </div>
                                <div class="space-y-1">
                                    <template x-for="change in log.changes">
                                        <div class="text-xs flex items-center gap-2">
                                            <span class="uppercase font-bold text-[10px] w-12 text-gray-500" x-text="change.type"></span>
                                            <div class="flex-1 flex items-center gap-2 font-mono text-gray-600">
                                                <span class="line-through text-red-400" x-text="change.old"></span>
                                                <i class="fas fa-arrow-right text-[8px] text-gray-300"></i>
                                                <span class="font-bold text-emerald-600" x-text="change.new"></span>
                                            </div>
                                        </div>
                                    </template>
                                </div>
                    ...
                    <div class="p-3 border-t bg-white text-center">
                        <button @click="clear" class="text-xs text-red-500 hover:text-red-700 font-bold">Clear History</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-data="{ open: false }" class="relative" @click.outside="open = false">
            <button @click="open = !open" class="flex items-center gap-2 hover:bg-slate-800 rounded-lg py-1 px-2 transition-colors">
                <div class="w-7 h-7 rounded-full bg-slate-700 flex items-center justify-center text-xs font-bold border border-slate-600 uppercase">{{ substr($userName, 0, 1) }}</div>
                <span class="hidden md:block text-sm font-medium capitalize">{{ $userName }}</span>
            ...
            <div x-show="open" x-cloak class="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-xl py-1 z-[100] border border-gray-100 animate-fade-in origin-top-right">
                <div class="px-4 py-2 border-b border-gray-100 text-xs text-gray-500">Logged in as <br><strong class="text-gray-800 capitalize">{{ $userName }}</strong></div>
  ...
  <script>
    ...
    // --- 3. MAIN APP CONTROLLER (UPDATED FOR AUTO-FOCUS) ---
    function app() {
        return {
            ...
            handleGlobalKeys(e) { 
                ...
                if(e.key === 'F2') { document.getElementById('product-search').focus(); this.mobileView = 'products'; } 
                ...
                if(e.key === '?') this.$dispatch('toggle-shortcuts'); 
                if(e.key === 'Escape') window.dispatchEvent(new CustomEvent('close-modals')); 

                // *** NEW AUTO-FOCUS LOGIC ***
                // If user scans or types, and they are NOT in a modal/input, focus search bar automatically
                const active = document.activeElement;
                const isInput = active.tagName === 'INPUT' || active.tagName === 'TEXTAREA' || active.isContentEditable;
                
                // Allow printable characters (len=1) to focus search
                if (!isInput && e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
                    const search = document.getElementById('product-search');
                    if (search) {
                        search.focus();
                        // We do NOT preventDefault, so the char gets typed into the now-focused input
                    }
                }
            },
    ...
    // --- 4. PERSISTENT HISTORY LOG COMPONENT ---
    const updateLogModal = {
        show: false, logs: [], unread: 0,
        init() {
            const stored = JSON.parse(localStorage.getItem('pos_update_logs') || '{"date":null, "data":[]}');
            const today = new Date().toDateString();
            if (stored.date !== today) { this.logs = []; localStorage.setItem('pos_update_logs', JSON.stringify({ date: today, data: [] })); } 
            else { this.logs = stored.data || []; }
            window.addEventListener('log-update', e => this.add(e.detail));
            window.addEventListener('close-modals', () => this.show = false);
        },
        add(data) {
            const entry = { name: data.name, changes: data.changes, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) };
            this.logs.unshift(entry); this.save(); 
            if(!this.show) this.unread++;
        },
        save() { const today = new Date().toDateString(); localStorage.setItem('pos_update_logs', JSON.stringify({ date: today, data: this.logs })); },
        openModal() { this.show = true; this.unread = 0; },
        close() { this.show = false; },
        clear() { this.logs = []; this.unread = 0; this.save(); }
    };

    // --- 5. PRODUCT GRID (CRITICAL SYNC LOGIC) ---
        ...
        return {
            ...
            init() {
                ...
                window.addEventListener('camera-scan', (e) => {
                    const code = e.detail;
                    const matches = this.all.filter(p => p.b == code);

                    if (matches.length === 1) {
                        // EXACT MATCH -> AUTO ADD
                        this.click(matches[0]);
                        ...
                        this.search = code;
                        this.updateFilter();
                        window.dispatchEvent(new CustomEvent('notify', { detail: { message: 'Multiple items found', type: 'success' } }));
                    }
                    else {
                        // NO MATCH
                        this.search = code;
                ...
                window.addEventListener('local-stock-update', (e) => {
                    e.detail.forEach(item => { const p = this.all.find(x => x.id == item.id); if (p) p.s = p.s - item.qty; });
                    this.updateFilter();
                });

                // START POLLING (Recursive to prevent stacking requests)
                this.pollUpdates();
            },

            async pollUpdates() {
                // Wait for next run
                const scheduleNext = () => setTimeout(() => this.pollUpdates(), 5000);
                    ...
                    if (data.products && data.products.length > 0) {
                        ...
                        data.products.forEach(newItem => {
                            ...
                            if (index !== -1) {
                                ...
                                const oldItem = this.all[index];
                                let changes = [];
                                
                                if(oldItem.n !== newItem.n) changes.push({type: 'Name', old: oldItem.n, new: newItem.n});
                                if(oldItem.p !== newItem.p) changes.push({type: 'Price', old: `₦${oldItem.p}`, new: `₦${newItem.p}`});
                                if(Math.abs(oldItem.s - newItem.s) > 1) changes.push({type: 'Stock', old: oldItem.s, new: newItem.s});
                                
                                if(newItem.v && newItem.v.length) {
                                    newItem.v.forEach(newV => {
                                        const oldV = oldItem.v ? oldItem.v.find(ov => ov.id == newV.id) : null;
                                        if(oldV && oldV.p !== newV.p) changes.push({type: 'Var Price', old: `${oldV.n}: ₦${oldV.p}`, new: `₦${newV.p}`});
                                    });
                                }

                                if(changes.length > 0) window.dispatchEvent(new CustomEvent('log-update', { detail: { name: newItem.n, changes: changes } }));
                                ...
                                window.dispatchEvent(new CustomEvent('log-update', { detail: { name: newItem.n, changes: [{type:'New', old:'-', new:'Added'}] } }));
                                hasChanges = true;
                                syncCount++;
                            }
                        });
                        ...
                        if (hasChanges) {
                            ...
                            let msg = `Synced: ${syncCount} updates`;
                            if(syncCount === 1) msg = "Product updated";
                            window.dispatchEvent(new CustomEvent('notify', { detail: { message: msg, type: 'success' } }));
                        }
                    }
                } catch (e) {
                    // console.error("Polling skipped due to error or offline");
                }
            ...
            enterSearch() {
                ...
                if (exact) { this.click(exact); }
            },
            click(p) {
                // *** CLEAR SEARCH ON CLICK ***
                if(this.search) { this.search = ''; this.updateFilter(); }
        ...
        return {
            ...
            init() { 
                ...
                window.addEventListener('sync-cart-items', e => {
                    ...
                    if (cartChanged) { this.save(); window.dispatchEvent(new CustomEvent('notify', { detail: { message: 'Cart prices updated!', type: 'error' } })); }
                });
    ...
    const mobileLinkModal = {
        ...
        startPolling() { if(this.poller) return; this.poller = setInterval(() => { if(!this.show) return; fetch("{{ route('scanner.fetch') }}?session_id=" + this.sessionId).then(res => res.json()).then(data => { if (data.scans && data.scans.length > 0) { this.connected = true; data.scans.forEach(scan => { playBeep(); window.dispatchEvent(new CustomEvent('camera-scan', { detail: scan.code })); }); } }); }, 1500); },
    ...
    const receiptModal={
        ...
        generateHtml(){
            ...
            document.getElementById('receipt-paper').innerHTML=`<div style="font-family: 'Space Mono', monospace; width: 100%; color: #000; font-size: 12px; line-height: 1.4;"><div style="text-align:center; margin-bottom: 10px;"><div style="font-size:16px; font-weight:800; text-transform:uppercase;">${STORE_INFO.name}</div>${STORE_INFO.company ? `<div style="font-size:10px; font-weight:bold; margin-top:2px;">${STORE_INFO.company}</div>` : ''}<div style="font-size:10px; margin-top:2px;">${STORE_INFO.address}</div>${STORE_INFO.phone ? `<div style="font-size:10px;">Tel: ${STORE_INFO.phone}</div>` : ''}${STORE_INFO.email ? `<div style="font-size:10px;">${STORE_INFO.email}</div>` : ''}${STORE_INFO.website ? `<div style="font-size:10px;">${STORE_INFO.website}</div>` : ''}</div><div style="border-bottom: 1px dashed #000; margin: 8px 0;"></div><div style="display:flex; justify-content:space-between; font-size:10px;"><span>${this.data.date}</span><span>Ref: ${this.data.id.substr(-6)}</span></div><div style="font-size:10px; margin-bottom:5px;">Method: ${this.data.method.toUpperCase()}</div><div style="border-bottom: 1px dashed #000; margin: 8px 0;"></div><div style="margin-bottom: 10px;">${itemsHtml}</div><div style="border-bottom: 1px dashed #000; margin: 8px 0;"></div><div style="display:flex; justify-content:space-between;"><span>Subtotal</span><span>₦${this.data.subtotal}</span></div>${parseFloat(this.data.discount.replace(/,/g,''))>0?`<div style="display:flex; justify-content:space-between;"><span>Discount</span><span>-₦${this.data.discount}</span></div>`:''}<div style="display:flex; justify-content:space-between; font-weight:800; font-size:16px; margin-top:5px;"><span>TOTAL</span><span>₦${this.data.total}</span></div><div style="text-align:center; margin-top:20px; font-size:10px;"><p>Cashier: ${this.data.original_user_name || 'Staff'}</p><p style="margin-top:10px; font-weight:bold;">${STORE_INFO.thank_you}</p><p style="margin-top:2px;">${STORE_INFO.visit_again}</p></div></div>`;
        },
    ...
    function syncManager() {
        return {
            ...
            // NEW: Right Click to clear queue
            forceClear() {
                if(confirm("Are you sure you want to delete all pending unsynced orders? This cannot be undone.")) {
                    this.queue = [];
                    this.save();
                    window.dispatchEvent(new CustomEvent('notify', { detail: { message: 'Queue Cleared Manually', type: 'error' } }));
                }
            },
            ...
            async process() {
                ...
                if(typeof order._retries === 'undefined') order._retries = 0;

                try {
                    // Safe Payload Construction
                    const payload = { 
                        cart: order.items.map(i => ({ 
                            product_id: (i.id && String(i.id).startsWith('custom')) ? null : i.id, 
                            custom_name: (i.id && String(i.id).startsWith('custom')) ? i.n : null, 
                            variant_id: i.vid, 
                            quantity: i.qty, 
                            price: i.p 
                        })), 
                        ...
                        discount: order.discount ? String(order.discount).replace(/,/g, '') : 0, 
                        total: order.total ? String(order.total).replace(/,/g, '') : 0, 
                        raw_total: order.raw_total, 
                        offline_user_id: order.original_user_id, 
                        offline_created_at: order.iso_date 
                    };
                    ...
                    if(res.ok || res.status === 422) { 
                        ...
                        if(res.status === 422) {
                             window.dispatchEvent(new CustomEvent('notify', { detail: { message: 'Order Skipped (Validation Error)', type: 'error' } })); 
                        } else {
                    ...
                    // Auto-remove if failed 3 times
                    if(order._retries >= 3) {
                        this.queue.shift();
                        this.save();
                        window.dispatchEvent(new CustomEvent('notify', { detail: { message: 'Corrupted order removed', type: 'error' } }));
                    }
                }
                
                this.syncing = false; 
                if(this.queue.length > 0 && this.online) setTimeout(() => this.process(), 1000);
            }
        }
    }
  </script>
