...
<body class="bg-gray-100 text-gray-800 h-full">
    ...
    <div class="flex">
        ...
        <nav id="sidebar" class="sidebar bg-purple-900 text-white h-screen w-64 p-5 fixed md:block transition-transform transform -translate-x-full md:translate-x-0">
            ...
            <ul class="space-y-2">
                ...
                <li>
    <a class="flex items-center p-2 rounded hover:bg-purple-700 {{ request()->is('stores*') ? 'bg-red-600' : '' }}" 
       href="{{ route('stores.index') }}"
       @if($isSubscriptionExpired) style="pointer-events: none; color: grey;" title="Subscription Expired" @endif>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Handle subscription expired status (Disable POS link)
        const posLink = document.getElementById("posLink");
        const isSubscriptionExpired = posLink ? posLink.getAttribute("data-subscription-expired") === "true" : false;

        if (isSubscriptionExpired && posLink) {
            // Disable the POS link and style it grey if the subscription is expired
            posLink.style.pointerEvents = "none";
            posLink.style.color = "grey";
            posLink.title = "Subscription Expired";  // Add tooltip for the expired subscription
        }
    });
</script>
