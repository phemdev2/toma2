...
<div class="subscription-card">
    <div class="plan">
        ...
        @elseif ($subscriptionType === 'basic')
            <span class="icon icon-basic">💼</span>
        @elseif ($subscriptionType === 'etc')
            <span class="icon icon-etc">💼</span>
        @elseif ($subscriptionType === 'premium')
            <span class="icon icon-premium">✨</span>
        @else
            <span class="icon icon-default">🌟</span> <!-- Default icon -->
        @endif
        <span>{{ ucfirst($subscriptionType) }} Plan</span>
    </div>

    @if ($isSubscriptionExpired)
        <p class="expired">Your subscription has expired. Please renew to access more features.</p>
        <button class="renew-btn">Renew Now</button>
    @else
        <p class="active">Your {{ ucfirst($subscriptionType) }} plan is active until: <strong>{{ $subscriptionExpiryDate }}</strong></p>
        ...
        <script>
            ...
            var x = setInterval(function() {
                ...
                if (distance < 0) {
                    ...
                    document.querySelector('.active').innerText = "Your subscription has expired.";
                    document.querySelector('.renew-btn').style.display = 'block'; // Show renew button if expired
                }
            }, 1000);
        </script>
    @endif
...
<script>
    // Dynamically set the subscription end date if available
    const endDate = new Date("{{ $trialEndDate }}");
    if (endDate != "Invalid Date") {
        const formattedEndDate = endDate.toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
        document.getElementById("endDate").innerText = "Your subscription ends on: " + formattedEndDate;
    }
</script>
@endsection
